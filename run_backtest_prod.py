...
def add_yoy(train_df, target_dates):
    ...
    for d in target_dates:
        ...
        vals = [idx.get(w, 0) for w in window]
...
def main():
    ...
    print(f"生产版回测: {len(skus)} SKU, 滚动窗口={ROLL_DAYS}天")
    print("协变量: is_promo, discount_rate, ppc_fee, day_of_week, is_weekend, month, qty_yoy\n")
    ...
    out_path = f'{OUT_DIR}/chronos2_backtest_prod.csv'
    df.to_csv(out_path, index=False)

    print(f"\n{'='*60}")
    print(f"生产版回测完成: {df['sku'].nunique()} SKU, {len(df)} 条记录")
